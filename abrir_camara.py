...
with LeptonSPI(SPI_DEVICE, speed_hz=SPI_SPEED_HZ) as cam:
    ...
    print(f"Frame capturado: {frame_raw.shape[1]}×{frame_raw.shape[0]} px")
    print(f"  Mín : {stats['min']:.1f} °C  en {stats['min_pos']}")
    print(f"  Máx : {stats['max']:.1f} °C  en {stats['max_pos']}")
    print(f"  Prom: {stats['avg']:.1f} °C")
    print("OK — cámara operativa.")
